...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="center",
        help="center uses one decimated FFT; multi-offset fuses all oversampling phases",
    )
    ...
    parser.add_argument("--phase-weight", type=float, default=0.10)
    ...
    for symbol in packet["payload_symbols"]:
        try:
            extractor = (
                ...
                if str(args.fft_evidence_mode) == "multi-offset"
                else extract_fft
            )
            spectra.append(
                extractor(
