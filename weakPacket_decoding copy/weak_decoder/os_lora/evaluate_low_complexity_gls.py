#!/usr/bin/env python3
"""Evaluate covariance-selected pattern banks with matrix-free cross-fit GLS."""
...
from weak_decoder.os_lora.analyze_empirical_noise_covariance import (  # noqa: E402
    _active_intervals,
    _empirical_covariance,
    _off_packet_starts,
)
from weak_decoder.os_lora.evaluate_nonuniform_sampling import _offpacket_bootstrap_samples  # noqa: E402
...
def _training_covariance(
    ...
    intervals = _active_intervals(packets, int(round(float(args.guard_symbols) * window_len)))
    ...
    bins = tuple(int(value) for value in np.linspace(0, n_bins, num=bin_count, endpoint=False, dtype=np.int64))
    downchirp = _oversampled_downchirp(sf, os_factor, 0, 0.0)
    vectors: list[np.ndarray] = []
    for start in starts:
        dechirped = np.asarray(samples[start: start + window_len] * downchirp, dtype=np.complex64)
        for raw_bin in bins:
            vectors.append(pattern_bin_values(dechirped, raw_bin, candidate_bank))
    return _empirical_covariance(np.asarray(vectors, dtype=np.complex128)), len(vectors)


def _build_selected_banks(
    candidate_bank: NonuniformPatternBank,
...
def _evaluate_conditional_condition(
    samples: np.ndarray,
    packets: Sequence[dict[str, Any]],
    banks: dict[str, NonuniformPatternBank],
    args: argparse.Namespace,
) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    """Measure only the deployable detector so every early exit is physical."""

    if args.branch_color_threshold is None:
        raise ValueError("--conditional-only requires --branch-color-threshold")
    detector_banks = {name: bank for name, bank in banks.items() if name.startswith("information_")}
    ...
    for method in methods:
        ...
        rows.append(
            {
                "method": str(method),
                "pattern_count": 0 if method == "savaux" else len(detector_banks[bank_name].names),
                "symbol_count": int(symbol_count),
                "errors": int(errors),
                "ser": float(errors / max(1, symbol_count)),
                "fixes_vs_savaux": int(fixes),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--candidate-kind", default="multiscale_only")
    parser.add_argument("--pattern-counts", nargs="+", type=int, default=[16, 32])
...
def main() -> int:
    ...
    candidate_bank = build_pattern_bank(
        int(packets[0]["sf"]),
