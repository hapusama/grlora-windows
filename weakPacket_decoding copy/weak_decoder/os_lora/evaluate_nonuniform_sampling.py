#!/usr/bin/env python3
"""Evaluate non-uniform OSR sampling evidence against Savaux OSR evidence."""
...
def _offpacket_pattern_covariance(
    samples: np.ndarray,
    ...
    bins = tuple(int(v) for v in np.linspace(0, n_bins, num=count, endpoint=False, dtype=np.int64))
    ...
    empirical = _empirical_covariance(np.asarray(train_vectors, dtype=np.complex128))
    test_covariance = _empirical_covariance(np.asarray(test_vectors, dtype=np.complex128))
    ...
    covariance += np.eye(covariance.shape[0], dtype=np.complex128) * max(mean_diag, 1e-30) * 1e-6
    ...
    cross_validated_replicas = float(
        raw_noise_power * abs(complex(weights.conj().T @ target)) ** 2
        / (float(n_bins) * max(test_noise, 1e-30))
    )
    white_replicas = float(np.median([effective_replica_count(bank, raw_bin) for raw_bin in bins]))
    return covariance, white_replicas, empirical_replicas, cross_validated_replicas, len(starts), bins


def _offpacket_bootstrap_samples(
    clean: np.ndarray,
    packets: Sequence[dict[str, Any]],
    snr_db: float | None,
    seed: int,
    ...
    source = np.asarray(
        [np.asarray(clean[int(start): int(start) + window_len], dtype=np.complex64) for start in starts],
        dtype=np.complex64,
    )
