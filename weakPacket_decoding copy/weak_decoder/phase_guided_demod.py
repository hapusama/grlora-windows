...
class PhaseGuidedPayloadConfig:
    ...
    # PHY-only scoring for a caller-supplied projected payload symbol sequence.
    # The sequence can come from the current packet's decoder beam or from a
    # controlled experiment; no session template, byte prior, or cross-packet
    # information is accepted here.
    projected_score_min_known: int = 8
    projected_score_phase_weight: float = 0.25
    projected_score_line_weight: float = 0.50
    projected_score_amp_weight: float = 0.20
    projected_score_profile_weight: float = 0.05
    projected_score_rmse_scale_pi: float = 0.30
    projected_score_mode: str = "bounded"
    projected_score_phase_kappa: float = 2.0
    projected_score_adaptive_kappa: bool = False
    projected_score_kappa_source: str = "scoring_line"
    projected_score_kappa_min: float = 1.0
    projected_score_kappa_max: float = 4.0
    projected_score_kappa_sigma_floor_pi: float = 0.12
    projected_score_amp_log_floor: float = 1e-6
...
def score_projected_payload_symbols(
    projected_symbols: Sequence[int],
    ...
    """Score one projected payload-symbol sequence against packet-local evidence.

    The score is intentionally not just a sum of FFT magnitudes. The projected
    canonical bins must also yield a smooth phase trajectory. The caller may
    provide a sequence generated by the current packet's decoder beam or by a
    controlled experiment; this helper does not learn or consume packet-structure
    templates, byte priors, or cross-packet state.
    """
    ...
    if score_positions:
        indexes = sorted(int(v) for v in score_positions)
            ...
            indexes = list(range(len(projected_symbols)))
    for k in indexes:
        if k < 0 or k >= len(projected_symbols):
            continue
        symbol = int(projected_symbols[k])
        if int(symbol) < 0:
            continue
    ...
    min_known = max(1, int(config.projected_score_min_known))
    ...
    if not math.isfinite(rmse_pi):
        ...
        scale = max(1e-6, float(config.projected_score_rmse_scale_pi))
    ...
    w_phase = max(0.0, float(config.projected_score_phase_weight))
    w_line = max(0.0, float(config.projected_score_line_weight))
    w_amp = max(0.0, float(config.projected_score_amp_weight))
    w_profile = max(0.0, float(config.projected_score_profile_weight))
    w_sum = w_phase + w_line + w_amp + w_profile
    if w_sum <= 0.0:
        w_phase, w_line, w_amp, w_profile, w_sum = 0.25, 0.50, 0.25, 0.0, 1.0

    score_mode = str(getattr(config, "projected_score_mode", "bounded")).lower()
    map_phase_ll = 0.0
    map_line_ll = 0.0
    map_amp_ll = 0.0
    map_profile_ll = 0.0
    effective_kappa = float(config.projected_score_phase_kappa)
    ...
    if bool(getattr(config, "projected_score_adaptive_kappa", False)):
        ...
        if math.isfinite(sigma_pi) and sigma_pi > 0.0:
            sigma_floor_pi = max(
                1e-6,
                float(getattr(config, "projected_score_kappa_sigma_floor_pi", 0.12)),
            )
            sigma_rad = max(sigma_floor_pi * math.pi, sigma_pi * math.pi)
            effective_kappa = float(1.0 / max(1e-12, sigma_rad * sigma_rad))
            kmin = max(0.0, float(getattr(config, "projected_score_kappa_min", 1.0)))
            kmax = max(kmin, float(getattr(config, "projected_score_kappa_max", 4.0)))
    ...
    if score_mode == "map":
        ...
        amp_floor = max(1e-12, float(config.projected_score_amp_log_floor))
