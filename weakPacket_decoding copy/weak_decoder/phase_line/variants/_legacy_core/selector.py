...
def _candidate_local_score(
    ...
    *,
    ...
    return float(
        ...
        + bonus
        + _reliable_top1_protection_bonus(ev, cand, config)
...
def _reliable_top1_protection_bonus(
    ev: SymbolEvidence,
    cand: _ViterbiCandidate,
    config: PhasePathSelectorConfig,
) -> float:
    if not bool(getattr(config, "reliable_top1_protection_enabled", False)):
        return 0.0
    if int(cand.raw_bin) != int(ev.top1_bin):
        return 0.0
    if float(ev.top1_margin_db) < float(getattr(config, "reliable_top1_protection_margin_db", 0.25)):
        return 0.0
    if float(ev.top1_peak_to_median_db) < float(
        getattr(config, "reliable_top1_protection_peak_to_median_db", 10.0)
    ):
        return 0.0
    if float(ev.top1_coherence_score) < float(getattr(config, "reliable_top1_protection_min_coherence", 0.88)):
        return 0.0
    return float(getattr(config, "reliable_top1_protection_bonus", 0.0))
...
def _viterbi_candidates(ev: SymbolEvidence, config: PhasePathSelectorConfig) -> tuple[_ViterbiCandidate, ...]:
    ...
    for rank, raw_bin in enumerate(bins):
        ...
        candidate = _ViterbiCandidate(
            ...
            rank_score=float(1.0 - float(rank) / float(rank_den)),
            phase_local_score=0.0,
        )
        high_confidence = _is_high_confidence(ev, config)
        local = _candidate_local_score(
            ev,
            candidate,
            config,
...
def _is_bidirectional_anchor(ev: SymbolEvidence, config: PhasePathSelectorConfig) -> bool:
    coherence_ok = (
        ev.offset_coherence is None
        or float(config.bidirectional_anchor_min_coherence) <= 0.0
        or float(ev.top1_coherence_score) >= float(config.bidirectional_anchor_min_coherence)
    )
    strict_anchor = bool(
        float(ev.top1_margin_db) >= float(config.bidirectional_anchor_margin_db)
        and float(ev.top1_peak_to_median_db) >= float(config.bidirectional_anchor_peak_to_median_db)
        and coherence_ok
    )
    if strict_anchor:
        return True
    return bool(config.bidirectional_lock_high_confidence and _is_high_confidence(ev, config))


def _pruned_bidirectional_anchor_mask(
    evidences: Sequence[SymbolEvidence],
    initial_mask: Sequence[bool],
    config: PhasePathSelectorConfig,
) -> tuple[bool, ...]:
    count = min(len(evidences), len(initial_mask))
    mask = [bool(initial_mask[idx]) for idx in range(count)]
    if not bool(config.bidirectional_anchor_prune_enabled):
        return tuple(mask)
    min_count = max(2, int(config.bidirectional_anchor_min_count))
    if sum(mask) <= min_count:
        return tuple(mask)

    anchor_items: list[tuple[int, float, float]] = []
    for idx, locked in enumerate(mask):
        if not locked:
            continue
        ev = evidences[idx]
        b = int(ev.top1_bin)
        if b < 0 or b >= ev.center_spectrum.size:
            continue
        anchor_items.append((idx, float(ev.abs_symbol_index), float(np.angle(ev.center_spectrum[b]))))
    if len(anchor_items) <= min_count:
        return tuple(mask)

    anchor_items.sort(key=lambda item: item[1])
    keep = np.ones(len(anchor_items), dtype=bool)
    threshold = max(0.0, float(config.bidirectional_anchor_prune_second_diff_pi)) * math.pi
    while int(np.sum(keep)) > min_count:
        active = np.flatnonzero(keep)
        if active.size < 3:
            break
        phases = np.unwrap(np.asarray([anchor_items[int(pos)][2] for pos in active], dtype=np.float64))
        second = np.abs(phases[2:] - 2.0 * phases[1:-1] + phases[:-2])
        if second.size == 0:
            break
        worst_local = int(np.argmax(second))
        worst_value = float(second[worst_local])
        if not math.isfinite(worst_value) or worst_value <= threshold:
            break
        keep[int(active[worst_local + 1])] = False

    out = [False for _ in range(count)]
    for pos, item in enumerate(anchor_items):
        if bool(keep[pos]):
            out[int(item[0])] = True
    return tuple(out)


def _bidirectional_phase_profile(
    evidences: Sequence[SymbolEvidence],
    anchor_mask: Sequence[bool],
) -> tuple[bool, np.ndarray]:
    count = min(len(evidences), len(anchor_mask))
    anchors: list[tuple[float, float]] = []
    for idx in range(count):
        if not bool(anchor_mask[idx]):
            continue
        ev = evidences[idx]
        b = int(ev.top1_bin)
        if b < 0 or b >= ev.center_spectrum.size:
            continue
        anchors.append((float(ev.abs_symbol_index), float(np.angle(ev.center_spectrum[b]))))
    if len(anchors) < 2:
        return False, np.zeros(len(evidences), dtype=np.float64)

    anchors.sort(key=lambda item: item[0])
    xs_anchor = np.asarray([item[0] for item in anchors], dtype=np.float64)
    phases_anchor = np.unwrap(np.asarray([item[1] for item in anchors], dtype=np.float64))
    xs_all = np.asarray([float(ev.abs_symbol_index) for ev in evidences], dtype=np.float64)
    profile = np.interp(xs_all, xs_anchor, phases_anchor)

    if xs_anchor.size >= 2:
        left = xs_all < xs_anchor[0]
        if np.any(left):
            dx = max(1e-9, float(xs_anchor[1] - xs_anchor[0]))
            slope = float((phases_anchor[1] - phases_anchor[0]) / dx)
            profile[left] = phases_anchor[0] + slope * (xs_all[left] - xs_anchor[0])
        right = xs_all > xs_anchor[-1]
        if np.any(right):
            dx = max(1e-9, float(xs_anchor[-1] - xs_anchor[-2]))
            slope = float((phases_anchor[-1] - phases_anchor[-2]) / dx)
            profile[right] = phases_anchor[-1] + slope * (xs_all[right] - xs_anchor[-1])
    return True, profile.astype(np.float64, copy=False)


def _bidirectional_candidate_bins(
    ev: SymbolEvidence,
    config: PhasePathSelectorConfig,
    locked: bool,
) -> tuple[int, ...]:
    ...
    bins: list[int] = []
    seen: set[int] = set()
    for raw_bin in ev.top_bins[: max(1, int(config.top_l))]:
        b = int(raw_bin)
        if b in seen or b < 0 or b >= ev.center_spectrum.size:
            continue
        if b != int(ev.top1_bin) and _candidate_drop_db(ev, b) < -float(config.bidirectional_max_energy_drop_db):
            continue
        bins.append(b)
        seen.add(b)
    if not bins:
        bins.append(int(ev.top1_bin))
    return tuple(bins)


def _bidirectional_score_candidate(
    ev: SymbolEvidence,
    raw_bin: int,
    predicted_phase: float,
    config: PhasePathSelectorConfig,
    ...
    coherence_score = _coherence_score(ev, b)
    phase = float(np.angle(ev.center_spectrum[b])) if 0 <= b < ev.center_spectrum.size else 0.0
    residual = float(wrap_phase(phase - float(predicted_phase)))
    phase_scale = max(1e-6, float(config.bidirectional_phase_scale_pi) * math.pi)
    phase_score = float(math.exp(-((residual / phase_scale) ** 2)))
    weights = np.asarray(
        [
            max(0.0, float(config.bidirectional_energy_weight)),
            max(0.0, float(config.bidirectional_coherence_weight)),
            max(0.0, float(config.bidirectional_phase_weight)),
        ],
        dtype=np.float64,
    )
    ...
    score = float(weights[0] * energy_score + weights[1] * coherence_score + weights[2] * phase_score)
    return score, energy_score, coherence_score, phase_score


def _is_island_anchor(ev: SymbolEvidence, config: PhasePathSelectorConfig) -> bool:
    coherence_ok = (
        ev.offset_coherence is None
...
def _pruned_island_anchor_mask(
    evidences: Sequence[SymbolEvidence],
    initial_mask: Sequence[bool],
    config: PhasePathSelectorConfig,
) -> tuple[bool, ...]:
    count = min(len(evidences), len(initial_mask))
    mask = [bool(initial_mask[idx]) for idx in range(count)]
    if not bool(config.island_anchor_prune_enabled):
        return tuple(mask)
    min_count = max(2, int(config.island_anchor_min_count))
    if sum(mask) <= min_count:
        return tuple(mask)
    ...
    anchor_items.sort(key=lambda item: item[1])
    keep = np.ones(len(anchor_items), dtype=bool)
    threshold = max(0.0, float(config.island_anchor_prune_second_diff_pi)) * math.pi
    while int(np.sum(keep)) > min_count:
        active = np.flatnonzero(keep)
        if active.size < 3:
            break
...
def _island_ranges(anchor_mask: Sequence[bool]) -> tuple[tuple[int, int], ...]:
    ranges: list[tuple[int, int]] = []
    start: int | None = None
    for idx, locked in enumerate(tuple(bool(v) for v in anchor_mask) + (True,)):
...
def _island_candidate_bins(
    ev: SymbolEvidence,
    config: PhasePathSelectorConfig,
) -> tuple[int, ...]:
    top_l = int(config.island_top_l) if int(config.island_top_l) > 0 else int(config.top_l)
    bins: list[int] = []
    seen: set[int] = set()
    for raw_bin in ev.top_bins[: max(1, top_l)]:
        b = int(raw_bin)
        if b in seen or b < 0 or b >= ev.center_spectrum.size:
            continue
        if b != int(ev.top1_bin) and _candidate_drop_db(ev, b) < -float(config.island_max_energy_drop_db):
            continue
        bins.append(b)
        seen.add(b)
    if not bins:
...
def _island_score_candidate(
    ev: SymbolEvidence,
    raw_bin: int,
    predicted_phase: float | None,
    config: PhasePathSelectorConfig,
) -> tuple[float, float, float, float]:
    b = int(raw_bin)
    energy_score = _energy_score(ev, b)
    coherence_score = _coherence_score(ev, b)
    phase_score = 0.0
    if predicted_phase is not None and 0 <= b < ev.center_spectrum.size:
        phase = float(np.angle(ev.center_spectrum[b]))
        residual = float(wrap_phase(phase - float(predicted_phase)))
        phase_scale = max(1e-6, float(config.island_phase_scale_pi) * math.pi)
        phase_score = float(math.exp(-((residual / phase_scale) ** 2)))
    weights = np.asarray(
        [
            max(0.0, float(config.island_energy_weight)),
            max(0.0, float(config.island_coherence_weight)),
            max(0.0, float(config.island_profile_weight if predicted_phase is not None else 0.0)),
        ],
        dtype=np.float64,
    )
    total = float(np.sum(weights))
    if total <= 0.0:
        ...
        weights = weights / total
    score = float(weights[0] * energy_score + weights[1] * coherence_score + weights[2] * phase_score)
    if int(b) == int(ev.top1_bin):
        score += max(0.0, float(config.island_top1_bonus))
    return score, energy_score, coherence_score, phase_score


def _island_anchor_candidate(ev: SymbolEvidence) -> _ViterbiCandidate:
    b = int(ev.top1_bin)
    phase = float(np.angle(ev.center_spectrum[b])) if 0 <= b < ev.center_spectrum.size else 0.0
    return _ViterbiCandidate(
        ...
        coherence_score=_coherence_score(ev, b),
        rank_score=1.0,
        phase_local_score=1.0,
    )


def _island_viterbi_range(
    evidences: Sequence[SymbolEvidence],
    lo: int,
    hi: int,
    anchor_mask: Sequence[bool],
    ...
    config: PhasePathSelectorConfig,
) -> tuple[tuple[int, ...], list[float], list[float], list[float], int]:
    left_anchor = _island_anchor_candidate(evidences[lo - 1]) if lo > 0 and bool(anchor_mask[lo - 1]) else None
    right_anchor = _island_anchor_candidate(evidences[hi]) if hi < len(evidences) and bool(anchor_mask[hi]) else None
    use_second_order = bool(int(config.phase_order) >= 2 and float(config.second_order_weight) > 0.0)
    transition_weight = max(0.0, float(config.island_transition_weight))
    boundary_weight = max(0.0, float(config.island_boundary_weight))

    candidate_lists: list[tuple[_ViterbiCandidate, ...]] = []
    candidate_count = 0
    for idx in range(int(lo), int(hi)):
        ev = evidences[idx]
        predicted = float(phase_profile[idx]) if phase_profile is not None and idx < phase_profile.size else None
        candidates: list[_ViterbiCandidate] = []
        bins = _island_candidate_bins(ev, config)
        rank_den = max(1, len(bins) - 1)
        for rank, raw_bin in enumerate(bins):
            b = int(raw_bin)
            score, energy_score, coherence_score, phase_score = _island_score_candidate(ev, b, predicted, config)
            phase = float(np.angle(ev.center_spectrum[b])) if 0 <= b < ev.center_spectrum.size else 0.0
            candidates.append(
                _ViterbiCandidate(
                    raw_bin=b,
        ...
        if not candidates:
            candidates.append(_island_anchor_candidate(ev))
    ...
    states: dict[tuple[int, int], tuple[float, tuple[int, ...], _ViterbiCandidate | None, _ViterbiCandidate]] = {}
    for cand in candidate_lists[0]:
        score = float(cand.local_score)
        if left_anchor is not None:
            score -= boundary_weight * _transition_penalty(None, left_anchor, cand, config)
        prevprev = left_anchor if left_anchor is not None else None
        key = (int(prevprev.raw_bin) if use_second_order and prevprev is not None else -1, int(cand.raw_bin))
        states[key] = (float(score), (int(cand.raw_bin),), prevprev, cand)

    for local_pos in range(1, len(candidate_lists)):
        next_states: dict[
            tuple[int, int],
            tuple[float, tuple[int, ...], _ViterbiCandidate | None, _ViterbiCandidate],
        ] = {}
        for score, path, prevprev, prev in states.values():
            for curr in candidate_lists[local_pos]:
                penalty_prevprev = prevprev if use_second_order else None
                penalty = transition_weight * _transition_penalty(penalty_prevprev, prev, curr, config)
                next_score = float(score + curr.local_score - penalty)
                key = (int(prev.raw_bin), int(curr.raw_bin)) if use_second_order else (-1, int(curr.raw_bin))
                item = (next_score, path + (int(curr.raw_bin),), prev, curr)
    ...
    if not states:
        selected = tuple(int(evidences[idx].top1_bin) for idx in range(int(lo), int(hi)))
    else:
        best_score = -float("inf")
        best_path: tuple[int, ...] = ()
        for score, path, prevprev, prev in states.values():
            ...
            if right_anchor is not None:
                penalty_prevprev = prevprev if use_second_order else None
                final_score -= boundary_weight * _transition_penalty(penalty_prevprev, prev, right_anchor, config)
            if final_score > best_score:
                best_score = final_score
                best_path = path
        selected = tuple(int(v) for v in best_path)

    local_scores: list[float] = []
    amp_scores: list[float] = []
    phase_scores: list[float] = []
    for rel_idx, raw_bin in enumerate(selected):
        ev = evidences[int(lo) + rel_idx]
        predicted = (
            float(phase_profile[int(lo) + rel_idx])
            if phase_profile is not None and int(lo) + rel_idx < phase_profile.size
            else None
        )
        score, amp, _coh, phase_score = _island_score_candidate(ev, int(raw_bin), predicted, config)
        local_scores.append(float(score))
        amp_scores.append(float(amp))
        phase_scores.append(float(phase_score))
    return selected, local_scores, amp_scores, phase_scores, int(candidate_count)


def select_anchor_bounded_island_viterbi_path(
    ...
    """Select payload bins with hard anchors and Viterbi only inside islands."""

    del fallback_line
    cfg = config or PhasePathSelectorConfig()
    stage1_config = SymbolPhaseConfig(
        top_l_low_confidence=max(1, int(cfg.top_l)),
    ...
    initial_anchor_mask = tuple(_is_island_anchor(ev, cfg) for ev in evidences)
    anchor_mask = _pruned_island_anchor_mask(evidences, initial_anchor_mask, cfg)
    if (
        sum(anchor_mask) < max(1, int(cfg.island_anchor_min_count))
        and not bool(getattr(cfg, "island_allow_unanchored_viterbi", False))
    ):
        ...
        return SymbolPhaseResult(
            ...
            locked_mask=anchor_mask,
            evidences=evidences,
            phase_line=line,
            trajectory_score=0.0,
            line_score=0.0,
            mean_phase_score=0.0,
            mean_amp_score=float(np.mean([_energy_score(ev, int(ev.top1_bin)) for ev in evidences])),
            uncertain_count=int(len(evidences) - sum(anchor_mask)),
            locked_count=int(sum(anchor_mask)),
            beam_final_size=0,
            error="island fallback to top1: insufficient anchors",
        )

    has_profile, profile = _bidirectional_phase_profile(evidences, anchor_mask)
    phase_profile = profile if has_profile else None
    selected_list = [int(ev.top1_bin) for ev in evidences]
    local_scores: list[float] = []
    amp_scores: list[float] = []
    phase_scores: list[float] = []
    candidate_count = 0
    for idx, locked in enumerate(anchor_mask):
        if not bool(locked):
            continue
        ev = evidences[idx]
        local_scores.append(float(_energy_score(ev, int(ev.top1_bin))))
        amp_scores.append(float(_energy_score(ev, int(ev.top1_bin))))
        phase_scores.append(1.0)

    for lo, hi in _island_ranges(anchor_mask):
        selected, scores, amps, phases, cand_count = _island_viterbi_range(
            evidences,
            int(lo),
            int(hi),
            anchor_mask,
        ...
        for rel_idx, raw_bin in enumerate(selected):
            selected_list[int(lo) + rel_idx] = int(raw_bin)
        local_scores.extend(scores)
        amp_scores.extend(amps)
        phase_scores.extend(phases)
        candidate_count += int(cand_count)

    selected = tuple(int(v) for v in selected_list)
    line = fit_selected_phase_line(
        center_spectra=[ev.center_spectrum for ev in evidences],
        selected_bins=selected,
        abs_indices=[ev.abs_symbol_index for ev in evidences],
    ...
    return SymbolPhaseResult(
        ...
        evidences=evidences,
        phase_line=line,
        trajectory_score=float(np.mean(local_scores)) if local_scores else 0.0,
        line_score=0.0,
        mean_phase_score=float(np.mean(phase_scores)) if phase_scores else 0.0,
        mean_amp_score=float(np.mean(amp_scores)) if amp_scores else 0.0,
        uncertain_count=int(len(evidences) - sum(anchor_mask)),
        locked_count=int(sum(anchor_mask)),
        beam_final_size=int(candidate_count),
        error="" if has_profile else "island ran without full phase profile",
    )


def select_anchor_bounded_bidirectional_rerank_path(
    center_spectra: Sequence[np.ndarray],
    evidence_powers: Sequence[np.ndarray],
    abs_indices: Sequence[float],
    config: PhasePathSelectorConfig | None = None,
    ...
    """Select payload bins using locked anchors and a bidirectional phase profile.

    Unlike the forward Viterbi path, this selector first locks only strict
    Stage-1 anchors, interpolates their unwrapped phases across the full
    payload, and then reranks every non-anchor symbol independently.
    """
    ...
    initial_anchor_mask = tuple(_is_bidirectional_anchor(ev, cfg) for ev in evidences)
    anchor_mask = _pruned_bidirectional_anchor_mask(evidences, initial_anchor_mask, cfg)
    if sum(anchor_mask) < max(2, int(cfg.bidirectional_anchor_min_count)):
        selected = tuple(int(ev.top1_bin) for ev in evidences)
        line = fit_selected_phase_line(
            center_spectra=[ev.center_spectrum for ev in evidences],
            selected_bins=selected,
        ...
        return SymbolPhaseResult(
            ...
            locked_count=int(sum(anchor_mask)),
            beam_final_size=0,
            error="bidirectional fallback to top1: insufficient anchors",
        )
    ...
    if not has_profile:
        ...
        return SymbolPhaseResult(
            ...
            locked_count=int(sum(anchor_mask)),
            beam_final_size=0,
            error="bidirectional fallback to top1: no phase profile",
        )
    ...
    candidate_count = 0
    for idx, ev in enumerate(evidences):
        locked = bool(idx < len(anchor_mask) and anchor_mask[idx])
        bins = _bidirectional_candidate_bins(ev, cfg, locked)
        candidate_count += len(bins)
        if locked:
            b = int(ev.top1_bin)
            selected_list.append(b)
            score, amp, _coh, phase_score = _bidirectional_score_candidate(ev, b, float(profile[idx]), cfg)
            local_scores.append(score)
            amp_scores.append(amp)
            phase_scores.append(phase_score)
            continue
        scored = [
            (raw_bin, *_bidirectional_score_candidate(ev, int(raw_bin), float(profile[idx]), cfg))
            for raw_bin in bins
        ]
        top1_score = next((item[1] for item in scored if int(item[0]) == int(ev.top1_bin)), scored[0][1])
        best = max(scored, key=lambda item: item[1])
        if int(best[0]) != int(ev.top1_bin) and float(best[1]) < float(top1_score) + float(cfg.bidirectional_min_switch_gain):
            best = next((item for item in scored if int(item[0]) == int(ev.top1_bin)), best)
        selected_list.append(int(best[0]))
        local_scores.append(float(best[1]))
        amp_scores.append(float(best[2]))
        phase_scores.append(float(best[4]))
