#!/usr/bin/env python3
"""Diagnose GT-vs-argmax ranking for linear fractional timing paths."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Timing-path GT-vs-argmax score diagnostic.")
    parser.add_argument("--dataset", default="0_0_0_10_14_16")
    parser.add_argument("--snr-db", type=float, default=-24.0)
    parser.add_argument("--output-dir", type=Path, default=WEAK_ROOT / "data" / "timing_path_oracle")
    parser.add_argument("--seed", type=int, default=20260531)
    parser.add_argument("--candidate-top-k", type=int, default=16)
    parser.add_argument("--tau-grid", default="-0.75,-0.5,-0.25,0,0.25,0.5,0.75")
    parser.add_argument("--slope-grid", default="-1.0,-0.5,0,0.5,1.0")
    parser.add_argument("--path-gain-power", type=float, default=0.20)
    parser.add_argument("--slope-penalty-power", type=float, default=0.10)
...
def main() -> int:
    ...
    tau_grid = _grid(args.tau_grid)
    slope_grid = _grid(args.slope_grid)
    ...
    for packet in load_savaux_packets(paths["symbols"], packet_filter=None, max_packets=None):
        ...
        for symbol in packet["payload_symbols"]:
            ...
            candidates = score_timing_path_candidates(
                ...
                slope_penalty_power=float(args.slope_penalty_power),
                savaux_power=power,
            )
            ...
            rows.append(
                {
                    ...
                    "gt_path_gain": float(gt_item.timing_path_gain),
                    "arg_path_gain": float(arg_item.timing_path_gain),
                    "gt_tau0": float(gt_item.best_tau0),
                    "arg_tau0": float(arg_item.best_tau0),
                    "gt_slope": float(gt_item.best_slope),
                    "arg_slope": float(arg_item.best_slope),
                    "would_fix": int(gt_bin != savaux_bin and gt_item.composite_score > arg_item.composite_score),
                }
            )

    wrong = [row for row in rows if not int(row["savaux_hit"])]
    summary = {
        ...
        "wrong_symbols": len(wrong),
        "would_fix_wrong_symbols": int(sum(int(row["would_fix"]) for row in wrong)),
        "mean_gt_score_over_arg_db_wrong": float(np.mean([float(row["gt_score_over_arg_db"]) for row in wrong])) if wrong else 0.0,
        "mean_gt_path_gain_wrong": float(np.mean([float(row["gt_path_gain"]) for row in wrong])) if wrong else 0.0,
        "mean_arg_path_gain_wrong": float(np.mean([float(row["arg_path_gain"]) for row in wrong])) if wrong else 0.0,
    }
    out = args.output_dir.resolve()
    out.mkdir(parents=True, exist_ok=True)
    _write_csv(out / f"{args.dataset}_snr_{int(args.snr_db)}_timing_oracle.csv", rows)
    (out / f"{args.dataset}_snr_{int(args.snr_db)}_timing_oracle_summary.json").write_text(
