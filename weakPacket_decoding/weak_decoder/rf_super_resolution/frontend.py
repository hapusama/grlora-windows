...
PyTorch and the external ``RFSuperResolution`` checkout are loaded only when
the frontend is instantiated. Importing weak-decoder therefore does not add a
mandatory ML dependency to the existing Savaux receiver.
"""
...
class RFSRFrontendConfig:
    """External checkout, checkpoint, and bounded-memory inference settings."""
